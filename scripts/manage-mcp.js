...
async function run() {
  ...
  const listUrl = new URL("admin/api/mcp/servers", baseUrl).toString();
  console.log("Fetching current MCP servers...");
  ...
  if (!response.ok) {
    console.error("Failed to fetch MCP servers:", response.status, await response.text());
    return;
  }
