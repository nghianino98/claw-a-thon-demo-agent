...
|---|---|---|---|
| A1 | 🔴 **NGHIÊM TRỌNG** | Repo Didi cũ từng chứa task/config có API key plaintext và đã được push lên remote. Không đưa chi tiết repo, số lượng key hoặc endpoint nội bộ vào tài liệu public. | (1) **Revoke/rotate toàn bộ token** liên quan; (2) gỡ các file dữ liệu chứa secret khỏi git tracking và thêm `data/` vào `.gitignore`; (3) purge history bằng `git filter-repo` rồi force-push repo đã làm sạch; (4) kiểm tra repo Settings/fork/collaborator bất thường. Việc này độc lập với mọi milestone — làm hôm nay |
